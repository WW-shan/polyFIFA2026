...
触发暂停后，要恢复到至少 **21 GiB** 空闲空间才会自动恢复，以免在阈值附近反复启停。`checkpoints/` 与 `runs/` 中有共享同一 inode 的硬链接，`du` 会把共享字节计到先遍历的目录；不能把 checkpoint 的目录大小当作可直接清理的重复副本。当前尚未实现日志压缩；直接把 `.ndjson` 改为 `.gz` 会破坏现有读取和快照引用，不应手工执行。
