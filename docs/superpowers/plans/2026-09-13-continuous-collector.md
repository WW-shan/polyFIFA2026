...
- [x] Locally integrate while preserving the two untracked user notes and existing raw samples. Install/start only this user's public collector on the chosen machine; verify native startup/stop/restart, main-project paths, owned process identity and status page.
- [x] Verify completed-game artifact serving against actual saved files. All four completed exports are readable; all 9,600 indexed Kichenok–Hibino rows were fetched individually with HTTP Range and matched the local depth file byte-for-byte. Preserve separate price/context quality labels.
- [ ] Resolve disk capacity without deleting raw evidence or lowering the safety reserve. Verify automatic resume, sustained new raw-file growth, both public streams, subsequent discovery and a new completed archive. Leave a genuinely collecting service running; an alive `paused_disk` process does not satisfy this gate.
...
- Two bootstrap collectors recorded several hours and were stopped gracefully only after the new supervisor had public WebSocket data. The foreground supervisor was then stopped gracefully before native deployment; its files remain under the main data root.
...
- Native LaunchAgent installation, post-merge real startup/stop/restart and owned PID identity were verified. On the 2026-09-14 19:04 Asia/Shanghai recheck, PID 57005 was alive but `paused_disk`, with about 20.03 GiB free, no connections and zero received records in the current process. The reserve is 20 GiB and post-pause resume requires 21 GiB. No raw captures were deleted or replaced, and compression is not implemented.
- Four automatically completed exports currently contain 6 price-ready directions in total, all from Kichenok–Hibino; none are strict-ready. Its complete 9,600-row HTTP Range verification covers valid, closed and invalid rows without relabeling them. The other three archives have incomplete/invalid tail seconds and remain unqualified.
- Fresh worktree typecheck passed. A first full test run, concurrent with typecheck, timed out in the existing locked-refill stress test; its isolated rerun and an unchanged full rerun passed (72 files / 1,999 tests). No timeout or assertions were weakened.
- The remaining operational gate is actual collection after capacity is resolved, not merely successful process installation. See `docs/continuous-collector.md` for the timestamped deployment evidence and limitations.
- Read-only storage audit: 77.13 GiB allocated collector tree; 73.26 GiB unique raw NDJSON across continuous/bootstrap runs. All checkpoint raw inodes already have run links. A 96 MiB, three-segment gzip/gunzip SHA-256 sample retained 9.42% of input size without writing compressed files or altering sources. This is sizing evidence only; full-file verified migration and reader/checkpoint compatibility remain unimplemented and require authorization before historical representation replacement.
