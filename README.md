...
Locked goal buys use an additional fake-goal guard. The watch loop caches one-goal-ahead locked orderbooks before the score changes, then after a score increase it requires a non-conflicting 365Scores goal signal plus stable S1/S2 orderbook delta before buying. 365Scores no-goal/VAR-disallowed signals, post-regulation/extra-time goal events, score conflicts, best-ask retrace, missing stable post-goal liquidity, implausibly low locked asks that a limit buy would cross, or isolated related-market movement block the locked buy; high return alone no longer creates a 5%/10% unconfirmed cap path.
